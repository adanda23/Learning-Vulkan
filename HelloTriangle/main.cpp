...
private:
    ...
    // manage memory used to store the buffers and command buffers are allocated from these pools
    ...
    void createCommandPool() {
        ...
        // allows command bufffers to be rerecorderd individually and often
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        // This function writes commands to the command buffer, which will be submitted to a queue for execution
        ...
        // Start a render pass, this will clear the framebuffer and allow us to draw on it
        VkRenderPassBeginInfo renderPassInfo{};
        renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass = renderPass;
        renderPassInfo.framebuffer = swapChainFramebuffers[imageIndex];

        // Define the area of the framebuffer to render to, in this case the entire framebuffer
        ...
        // Bind the graphics pipeline to the command buffer, this will tell the command buffer to use our pipeline for rendering commands
        ...
        // Draw a triangle, this will use the vertex shader to generate the vertices of the triangle and the fragment shader to color it
    ...
    void createRenderPass() {
        ...
        // wont be using stencil buffer so put dont care
        colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;

        // We want image to be ready for presentation using swap chain after rendering
        colorAttachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        colorAttachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
    
        // A render pass consists of subpasses which are rendering operations
        // that depend on the contents of framebuffer in previous passes
        // Reference to the attachment in the subpass
   ...
   void createGraphicsPipeline() {
    ...
    void createFramebuffers() {
        // Framebuffers are the attachments that a render pass instance uses, in this case they are the swapchain image views
    ...
    void drawFrame() {
        ...
        // Semaphores are used to synchronize operations between the CPU and GPU, and between different GPU operations
        // Fences are used to synchronize the CPU with the GPU, they can be used to wait for the GPU to finish work before the CPU continues

        // we will use semaphores to ensure that the image is available before we draw on it and that drawing is finished before we present it
